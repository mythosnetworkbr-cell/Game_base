...
stock NYX_FireSpawn()
{
    ...
    for (new playerid = 0; playerid < MAX_PLAYERS; playerid++)
    {
        if (IsPlayerConnected(playerid) && IsOnFireDuty[playerid])
            SendClientMessage(playerid, 0xFF6B35FF, "[BOMBEIROS] Incendio detectado! Use /apagar.");
    }
    return 1;
}
...
stock NYX_FireDuty(playerid)
{
    ...
    {
        SendClientMessage(playerid, 0x66FF99FF, "[BOMBEIROS] Servico iniciado.");
        if (!FireData[fActive]) NYX_FireSpawn();
    }
    else SendClientMessage(playerid, 0xFFCC00FF, "[BOMBEIROS] Servico encerrado.");
    return 1;
}
